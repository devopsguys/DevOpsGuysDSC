...
function Throw-InvalidJreInstallerException
{
    param ([string] $Message)

    $exception = new-object System.ArgumentException $Message
    $errorRecord = New-Object System.Management.Automation.ErrorRecord $exception,'MSINotJavaInstaller','InvalidArgument',$null
    throw $errorRecord
}
...
Function Validate-StandardArguments
{
    ...
    Trace-Message "Validate-StandardArguments, Path was $Path, Version was $Version"
    $uri = $null
    try
    {
        $uri = [uri] $Path
    ...
    {
        Throw-InvalidArgumentException ($LocalizedData.InvalidVersion -f $Version) 'Version'
    }

    return $uri, $v
}

Function Get-OracleJreProduct
{
    $uninstallKey = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    $uninstallKeyWow64 = 'HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall'

    $jrePattern = (Get-OracleJrePatterns) -join '|'

    $getVersionFromDisplayVersion = { Get-VersionFromString -String (Get-LocalizableRegKeyValue -RegKey $_ -ValueName DisplayVersion) }

    Get-ChildItem -ErrorAction Ignore $uninstallKey, $uninstallKeyWow64 |
    Where-Object Name -Match "\\\{(?:$jrePattern)[^\\]*\}$" |
    Sort-Object -Property $getVersionFromDisplayVersion -Descending |
    Select-Object -First 1
}

function Get-OracleJrePatterns
{    
    '26A24AE4-039D-4CA4-87B4-2F8'
    '3248F0A8-6813-11D6-A77B-00B0D0'
    '7148F0A8-6813-11D6-A77B-00B0D0'
}
...
function Test-TargetResource 
{
    ...
    $uri, $v = Validate-StandardArguments -Path $Path -Version $Version
    $product = Get-OracleJreProduct
    ...
    {
        $installedVersion = Get-VersionFromString -String (Get-LocalizableRegKeyValue -RegKey $product -ValueName DisplayVersion)
        return $Ensure -eq 'Present' -and $null -ne $installedVersion -and $installedVersion -ge $Version
    }
}
...
function Get-LocalizableRegKeyValue
{
    ...
    $res = $RegKey.GetValue("${ValueName}_Localized")
    if(-not $res)
    {
...
function Get-TargetResource
{
    ...
    $product = Get-OracleJreProduct -ExactVersion $Version
    ...
    $installedVersion = $product.GetValue('DisplayVersion')
...
function Set-TargetResource 
{
    ...
    try
    {
        ...
        if ($Ensure -eq 'Present')
        {
            if($fileExtension -eq '.msi')
            {
                ...
                $isKnownJre = $false
                foreach ($pattern in Get-OracleJrePatterns)
                {
                ...
                if (-not $isKnownJre)
                {
                    Throw-InvalidJreInstallerException ($LocalizedData.InvalidJreInstaller)
                }
    ...
    if($Ensure -eq 'Present')
    {
        $productEntry = Get-OracleJreProduct
        if(-not $productEntry)
        {
